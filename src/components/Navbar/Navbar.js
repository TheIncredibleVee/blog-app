...
const Navbar = () => {
    ...
    const {isLoggedIn, error, signOut, authError} = useContext(UserContext);
    ...
    return (
        ...
        <div className="flex md:order-2">
            {isLoggedIn && !authError?<Link to="/"><button type="button" className="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-3 md:mr-0 bg-red-600 hover:bg-red-700 focus:ring-red-800" onClick={signOut}>Logout</button></Link>:<Link to="/login"><button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-3 md:mr-0 bg-blue-600 hover:bg-blue-700 focus:ring-blue-800" onClick={handleLogin}>Login</button></Link>}
        ...
        <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="mobile-menu-4">
            ...
            <li>
                <Link to="/" className={path==='/'?active:inActive} aria-current="page">Home</Link>
            </li>
            <li>
                <Link to="/dashboard" className={path==='/dashboard'?active:inActive}>Dashboard</Link>
            </li>
            <li>
                <Link to="/" className={path==='/services'?active:inActive}>Services</Link>
            </li>
            <li>
